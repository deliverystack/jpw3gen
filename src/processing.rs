...
pub fn markdown_to_html(
    ...
    content_for_normalization = control_char_regex
        .replace_all(&content_for_normalization, "")
        .to_string()
        .replace('\r', "")
        .replace(['\u{2011}', '\u{2013}'], "-") // Non-breaking hyphen and En dash
        .replace('\u{2014}', "--") // Em dash
        .replace('\u{00A0}', " ") // Non-breaking space
        // Smart quotes to straight quotes
        .replace(['\u{2018}', '\u{2019}', '\u{201A}', '\u{201B}'], "'") // Left single quote, Right single quote, Single low quote, Single high reversed quote
        .replace(['\u{201C}', '\u{201D}', '\u{201E}', '\u{201F}'], "\"") // Left double quote, Right double quote, Double low quote, Double high reversed quote
        // Ellipsis
    ...
    let lines_to_convert: Vec<String> = content_for_normalization
        .lines()
        .map(|line| {
    ...
    // 3. TODO FORMATTING
    let lines_with_todo: Vec<String> = content_for_normalization
        .lines()
        .map(|line| {
    ...
    let starts_with_whitespace = content_for_normalization
        .chars()
        .next()
    ...
    let (html_output_content, title_from_h1) =
